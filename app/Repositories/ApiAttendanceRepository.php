...
class ApiAttendanceRepository
{
    ...
    {
        $recent_data = EmployeeAttendance::where('finger_print_id', $data->finger_id)->whereDate('in_out_time', date('Y-m-d', \strtotime($data->datetime)))->latest()->first();

        if (!$recent_data) {
            $employeeData['check_type'] = 'IN';
        } elseif (isset($recent_data['check_type']) && $recent_data['check_type'] == 'OUT') {
            $employeeData['check_type'] = 'IN';
        } else {
            $employeeData['check_type'] = 'OUT';
        }
