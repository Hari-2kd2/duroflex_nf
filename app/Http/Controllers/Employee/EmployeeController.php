...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $subDepartmentList = SubDepartment::get();
        $costCenterList = CostCenter::get();
        ...
        $results = Employee::with(['userName' => function ($q) {
            $q->with('role');
        }, 'department', 'designation', 'branch', 'payGrade', 'supervisor', 'hourlySalaries', 'costcenter', 'subdepartment'])
            ->orderBy('employee_id', 'DESC')->where('status', '!=', UserStatus::$TERMINATE)->get();

        if (request()->ajax()) {
            if ($request->role_id != '') {
                $results = Employee::whereHas('userName', function ($q) use ($request) {
                    $q->with('role')->where('role_id', $request->role_id);
                })->with('department', 'designation', 'branch', 'payGrade', 'supervisor', 'hourlySalaries', 'costcenter', 'subdepartment')->orderBy('employee_id', 'DESC');
            } else {
                $results = Employee::with(['userName' => function ($q) {
                    $q->with('role');
                }, 'department', 'designation', 'branch', 'payGrade', 'supervisor', 'hourlySalaries', 'costcenter', 'subdepartment'])->orderBy('employee_id', 'DESC');
            }
        ...
        return view('admin.employee.employee.index', ['costCenterList' => $costCenterList, 'subDepartmentList' => $subDepartmentList, 'results' => $results, 'departmentList' => $departmentList, 'designationList' => $designationList, 'roleList' => $roleList]);
    }
    ...
    public function subdept(Request $request)
    {

        $sub_department = SubDepartment::where('department_id', $request->department_id)->get();

        $output = '';
        $output = "<option value=''>---Please Select ---</option>";

        foreach ($sub_department as $key => $value) {
            $output .= "<option value=" . $value->sub_department_id . "> " . $value->sub_department_name . " </option>";
        }
    ...
    public function costcenter(Request $request)
    {
        ...
        $cost_center = CostCenter::where('sub_department_id', $request->sub_department_id)->get();
        ...
        foreach ($cost_center as $key => $value) {
            ...
            $output .= "<option value=" . $value->cost_center_id . "> " . $value->cost_center_number . " </option>";
        }
    ...
    public function export()
    {
        ...
        $employees = Employee::where('status', UserStatus::$ACTIVE)->with('department', 'branch', 'designation', 'costcenter', 'subdepartment', 'workshift', 'userName', 'supervisor')->get();
        // dd($employees);
        ...
        foreach ($employees as $key => $Data) {
            ...
            if (isset($Data->workshift->shift_name)) {
                $shift_name = $Data->workshift->shift_name;
            }
            ...
            $dataset[] = [
                ...
                $Data->subdepartment->sub_department_name,
                $Data->costcenter->cost_center_number,
                ...
                $Data->service_charge == 1 ? 'Enabled' : 'Disabled',
            ];
