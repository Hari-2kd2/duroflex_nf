...
class MobileAttendanceController extends Controller
{
    public function mobileAttendance(Request $request)
    {
        ...
        //     ->where('view_employee_in_out_data.device_name', 'mobile')
        ...
        if ($request->date) {
            ...
            $attendanceData = EmployeeInOutData::join('employee', 'employee.finger_id', 'view_employee_in_out_data.finger_print_id')
                ...
                ->join('designation', 'designation.designation_id', 'employee.designation_id')
                ->where('employee.status', 1)
                ->where('view_employee_in_out_data.date', Carbon::createFromFormat('d/m/Y', $request->date)->format('Y-m-d'))
                ->where('view_employee_in_out_data.device_name', 'mobile')
                ->groupBy('employee.finger_id')
                ->get();
            // dd($request->date);
            return View('admin.attendance.mobile.index', ['attendanceData' => $attendanceData, 'date'=> $request->date])->render();
        }

        return \view('admin.attendance.mobile.index', ['attendanceData' => $attendanceData, 'date' => $request->date]);
    }
    ...
    public function mobileAttendanceReport(Request $request)
    {
        ...
        $employeeInfo = EmployeeAttendance::join('employee', 'employee.employee_id', 'employee_attendance.employee_id')
            ...
            ->join('designation', 'designation.designation_id', 'employee.designation_id')
            ->where('employee_attendance.employee_id', $employee_id)->groupBy('employee_attendance.employee_attendance_id')
            // ->whereDate('view_employee_in_out_data.date', $date)
            ->where('employee_attendance.uri', 'api')->select('employee_attendance.*')
        ...
        foreach ($employeeInfo as $key => $value) {
            ...
            if ($value->check_type == 'IN') {
                ...
                $address                               = $this->getaddress($value->latitude, $value->longitude);
                $attDataFormat[$i]['address']          = $address;
                $map_dataset[]                         = ["timestamp" => $address, "latitude" => $value->latitude, "longitude" => $value->longitude];
            }
