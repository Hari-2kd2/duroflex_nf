...
class EmployeeAttendaceControllerSample extends Controller
{
    ...
    public function fetchRawLog()
    {
        ...
        if ($lastLogRow) {
            $LogCollections = DB::connection('mysql2')
                ->table($table_name)
        ...
        foreach ($LogCollections as $key => $log) {
            ...
            $check_record = DB::table('ms_sql')
                ->where('ID', $log->USRID)
                ->where('evtlguid', $log->EVTLGUID)
                ->where('devdt', $log->DEVDT)
                ->first();
            ...
            if (!$last_record) {
                $type = 'IN';
            } elseif ($last_record && $last_record->type == 'OUT') {
                $type = 'IN';
            } else {
                $type = 'OUT';
            }
    ...
    public function attendance($finger_print_id = null, $manualAttendance = false, $manualDate = null)
    {
        ...
        if ($finger_print_id != null && $manualAttendance != false && $manualDate != null) {
            $secondRun = true;
            $date = $manualDate;
            $employees = Employee::where('finger_id', $finger_print_id)
                ->select('finger_id', 'employee_id')
                ->get();
        } else {
            $employees = Employee::select('finger_id', 'employee_id')
                ->groupby('finger_id')
                ->get();
        }
        ...
        foreach ($employees as $key1 => $finger_id) {
            ...
            $start_date = DATE('Y-m-d', strtotime($date)) . ' 06:30:00';
            $end_date = DATE('Y-m-d', strtotime($date . ' +1 day')) . ' 08:30:00';

            // dd($rework, $secondRun,$start_date, $end_date );
            ...
            if ($data_format != [] && isset($data_format['working_time'])) {
                ...
                if ($data_format != [] && isset($data_format['in_time']) && isset($data_format['out_time'])) {
                    foreach ($shift_list as $key => $value) {
                        ...
                        $buffer_start_time = Carbon::createFromFormat('H:i:s', $value->start_time)
                            ->subMinutes(29)
                            ->format('H:i:s');
                        $buffer_end_time = Carbon::createFromFormat('H:i:s', $value->start_time)
                            ->addMinutes(29)
                            ->format('H:i:s');
            ...
            if ($data_format != [] && (isset($data_format['working_time']) || isset($data_format['in_time']) || isset($data_format['out_time']))) {
                ...
                if ($workingTime[0] >= 0) {
                    $if_exists = EmployeeInOutData::where('finger_print_id', $data_format['finger_print_id'])
                        ->where('date', $data_format['date'])
                        ->first();
                    ...
                    if (!$if_exists) {
                        ...
                        //  $data_format['attendance_status'] = AttendanceStatus::$PRESENT;
                        EmployeeInOutData::where('date', $data_format['date'])
                            ->where('finger_print_id', $data_format['finger_print_id'])
                            ->update($data_format);
                    }
                }
            } else {
                $if_exists = EmployeeInOutData::where('finger_print_id', $finger_id->finger_id)
    ...
    public function calculate_attendance($date_from, $date_to, $finger_id, $reRun)
    {
        ...
        if (count($results) == 1 && $results[0]->type == 'IN') {
            ...
            if ($count_check == count($results)) {
                ...
                $update = DB::table('ms_sql')
                    ->whereIn('primary_id', $primary_id)
                    ->update(['status' => 1]);
                ...
                $update = DB::table('ms_sql')
                    ->whereIn('primary_id', $primary_id_2)
                    ->update(['status' => 1]);
                ...
                $update = DB::table('ms_sql')
                    ->whereIn('primary_id', $primary_id_3)
                    ->update(['status' => 1]);
