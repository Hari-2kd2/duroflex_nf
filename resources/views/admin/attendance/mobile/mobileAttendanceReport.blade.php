...
<div class="container-fluid">
    <div class="row bg-title">
        ...
        <div class="text-right">
            <a href="{{ route('mobileAttendance.mobileAttendance') }}" style="margin-right: 20px" class="btn btn-info btn-sm">
                <span class="text-white text-right">
                    <span class="text-white" style="font-size: 16px;">
                        </span> BACK TO MOBILE ATTENDANCE
                    </span> </a>
        </div>
    </div>
    <div class="row">
                                    ...
                                    <tbody>
                                        @if (count($employeeInfo) > 0)
                                            <tr class="text-center">
                                                <td style="padding-top: 50px;padding-bottom:50px;width:50px">
                                                    {{ $loop->iteration }}
                                                </td>
                                                <td>
                                                    @if (isset($value['face_id_in']) && $value['face_id_in'] != '')
                                                        <img id="face_image"
                                                            ...
                                                            class="img-circle">
                                                    @elseif($value['face_id_in'] == '' && isset($value['face_id_out']) && $value['face_id_out'] != '')
                                                        <img style=" width: 100px; " src="{!! asset('admin_assets/img/default.png') !!}"
                                                ...
                                                <td style="padding-top: 50px;padding-bottom:50px">
                                                    {{ date('Y-m-d', strtotime($value['in_time'])) }}
                                                </td>
                                                ...
                                                <td class="text-center" style="padding-top: 32px;padding-bottom:32px">
                                                    {{ 'Name : ' . $value['first_name'] }}<br>
                                                    {{ 'Department : ' . $value['department_name'] }} <br>
                                                    {{ 'Designation : ' . $value['designation_name'] }}</td>

                                                <td style="padding-top: 50px;padding-bottom:50px">
                                                    @if (isset($value['check_in']) && isset($value['in_time']))
                                                        {{ date('H:i', strtotime($value['in_time'])) . ' : ' . $value['check_in'] }}
                                                    @elseif (isset($value['in_time']))
                                                        {{ date('H:i', strtotime($value['in_time'])) . ' : ' . 'Web' }}
                                                    @else
                                                        {{ '--' }}
                                                    @endif
                                                </td>
                                                ...
                                                <td style="padding-top: 50px;padding-bottom:50px">
                                                    @if (isset($value['check_out']) && isset($value['out_time']))
                                                        {{ date('H:i', strtotime($value['out_time'])) . ' : ' . $value['check_out'] }}
                                                    @elseif (isset($value['out_time']))
                                                        {{ date('H:i', strtotime($value['out_time'])) . ' : ' . 'Web' }}
                                                    @else
                                                        {{ '--' }}
                                                    @endif
                                                </td>

                                                <td style="padding-top: 12px;padding-bottom:12px; width:150px">
                                                    {{ $value['address'] }}</td>
                                                ...
                                                <td class="text-center">
                                                    @if (isset($value['check_in']) && isset($value['lat_in']) && isset($value['lng_in']))
                                                        <div class="">
                                                            <div class="">
                                                                <iframe width="" height="100" frameborder="0"
                                                                    style="border:0"
                                                    ...
                                                    @else
                                                        <img class="" style="width: 300px; height: 100px"
                                                            src="{!! asset('admin_assets/img/404_1.png') !!}">
                                                    @endif
                                                </td>
