        ...
        {{-- <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <a href="{{ route('generateSalarySheet.create') }}"
                class="btn btn-success pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light"> <i
                    class="fa fa-plus-circle" aria-hidden="true"></i> @lang('salary_sheet.generate_salary_sheet')</a>
        </div> --}}
    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    {{ Form::select('payment_method', ['' => '---- ' . __('common.please_select') . ' ----', 'Cash' => 'Cash', 'Cheque' => 'Cheque', 'RazorPay' => 'RazorPay'], '', ['class' => 'form-control payment_method select2 required']) }}
                                </div>
                            </div> --}}
                            <div class="ml-auto" style="padding: 24px 24px;">
                                {{-- <a href="{{ route('generateSalarySheet.create') }}"
                                    ...
                                    <span class="text-white" style="padding-left: 4px">Make Payment To
                                        All</span></a> --}}
                                <a href="{{ route('generateSalarySheet.create') }}"
                                    class="btn btn-info pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light">
                                    <i class="fa fa-plus-circle text-white" aria-hidden="true"></i>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            {{-- <div class="panel-body text-center">
                <div class="panel-body text-center">
                    <form action="{!! Url('generateSalarySheet/razorpay') !!}" method="POST"> --}}
                        {{-- <button type="button" id="makeDigitalPayment"
                            class="btn btn-info btn_style waves-effect waves-light makeDigitalPayment"
                            data-dismiss="modal">
                            <b>RazorPay</b></button> --}}
                        <!-- Note that the amount is in paise = 50 INR -->
                        <!--amount need to be in paisa-->
                        {{-- <script src="https://checkout.razorpay.com/v1/checkout.js" id="makeDigitalPayment" data-key="rzp_test_EzVAI0XNlc8bPq"
                                                data-amount="1000" data-buttontext="Pay IN INR" data-name="Monthly Salary"
                                                data-description="Total Amount" data-image="yout_logo_url" data-prefill.name="name"
...
<script>
    $(function() {
        $(document).on('click', '[data-salary_details_id]', function(event) {
            ...
            if (total_allowance == 0 && basic_salary == 0 && total_deduction == 0) {
                $('.basic_salary').parent().css({
                    "display": "none"
                });
                $('.total_allowance').parent().css({
                    "display": "none"
                });
                $('.total_deduction').parent().css({
                    "display": "none"
                });
        ...
        $(".status,.monthField,.paymentMethod").change(function() {
            getData(1);
        });
    ...
    function getData(page) {
        ...
        $.ajax({
            url: '?page=' + page + "&status=" + status + "&monthField=" + monthField  + "&payment_method=" + payment_method,
            datatype: "html",
        }).done(function(data) {
            $('.data').html(data);
            $("html, body").animate({
