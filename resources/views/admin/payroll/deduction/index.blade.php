        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <a href="{{ route('deduction.create') }}"
                class="btn btn-success pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light"> <i
                    class="fa fa-plus-circle" aria-hidden="true"></i> @lang('deduction.add_deduction')</a>
        </div>
    </div>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($results as $value)
                                        <tr class="{!! $value->deduction_id !!}">
                                            ...
                                            <td>
                                                @if ($value->deduction_type == 'Percentage')
                                                    {!! $value->percentage_of_basic !!}%
                                                @else
                                                {!! 'Not Applicable' !!}    
                                                @endif
                                            </td>
                                            @if ($value->deduction_criteria == 0)
                                            <td>{!! 'From Gross Salary' !!}</td>
                                            @else
                                            <td>{!! 'From Basic Salary' !!}</td>
                                            @endif
                                            <td>{!! $value->limit_per_month !!}</td>
                                            <td style="width: 100px;">
                                                <a href="{!! route('deduction.edit', $value->deduction_id) !!}"
                                                    class="btn btn-success btn-xs btnColor">
                                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                                </a>
                                                <a href="{!! route('deduction.delete', $value->deduction_id) !!}" data-token="{!! csrf_token() !!}"
