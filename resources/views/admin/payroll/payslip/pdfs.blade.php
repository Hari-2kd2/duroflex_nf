...
@foreach ($collections as $payroll)
    @php
        $employee = App\Model\Employee::find($payroll->employee);
    @endphp
    ...
    <table cellpadding="5" cellspacing="5" class="border1 width100 b-top0">
        <tr>
            <td class="al-center">WAGE SLIP for the month of
                {{ strtoupper(date('M-y', strtotime($payroll->year . '-' . $payroll->month))) }}
            </td>
        </tr>
    </table>
    <table cellpadding="10" cellspacing="10" class="table-bordered border1 width100 b-top0 b-collapse" border="1">
        <tr>
            ...
            <td class="width18">
                {{ number_format((float) $payroll->no_day_wages + $payroll->company_holiday, 2, '.', '') }}
            </td>
        </tr>
        <tr>
            <td>Employee ID </td>
    ...
    <p class="al-center" style="padding-bottom: 92px;">This is computer generated payslip. No Signature is required.
    </p>
    ...
    <table cellpadding="10" cellspacing="10" class="border1 width100 b-top0 b-collapse" border="1">
        <tr>
            ...
            <td class="width21">{{ number_format((float)$payroll->no_day_wages + $payroll->ph + $payroll->company_holiday, 2, '.', '') }}
            </td>
        </tr>
        <tr>
            <td>Employee ID </td>
