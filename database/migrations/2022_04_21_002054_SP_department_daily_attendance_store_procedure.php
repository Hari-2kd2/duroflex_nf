...
JOIN work_shift on work_shift.work_shift_id = employee.work_shift_id
    where (`status`=1 AND `date`=input_date)AND(employee.department_id=department_id OR department_id="") GROUP BY view_employee_in_out_data.finger_print_id ORDER BY employee_attendance_id DESC;
